...
import {  onAddToCart, onRemoveProduct, onCartEmpty } from '../lib/features/cart/cartSlice';
...
const page = () => {
  ...
  const dispatch = useDispatch();
   ...
   const handleAddToCart = (product:string , qnt: number) => {
        dispatch(onAddToCart({ productId: product, quantity: qnt }));
      };
  ...
  if(cartItem.length === 0){
    return <div className='flex flex-col gap-5 items-center justify-center min-h-screen'>
              ...
              <h1 className='text-4xl font-mono font-light'>Cart is Empty</h1>  <Link href="/" className='px-2 py-1 shadow-lg hover:scale-75 text-md font-serif duration-300 bg-orange-500 rounded-lg text-white'>Add products</Link>
              </div>
  }
  return (
    <div className='flex items-center min-h-screen flex-col pt-20'>
      {/* {
        ...
        <div className='flex items-center justify-end mt-8  gap-4'>
            ...
            <Link href="/checkout"><button className='w-32 py-2 bg-cyan-700 text-white rounded shadow-lg hover:scale-75 duration-300 text-xl font-semibold'>Checkout</button></Link>
        </div>
        </div>
  )
}
